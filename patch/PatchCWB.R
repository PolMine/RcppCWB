...
#            remove_lines = list("(\\s+)stderr,", 1),
        ...
        "src/cwb/cqp/parser.tab.c" = c(
          list(
            ...
            replace = list("^\\s*#define\\sIPAddress\\s267$", "#define _IPAddress 267", 1L),
        ...
        "src/cwb/CQi/server.c" = c(
          list(
            ...
            #   /usr/x86_64-w64-mingw32/sys-root/mingw/include/winsock2.h:1027:60: note: expected 'const char *' but argument is of type 'unsigned char *'
        ...
        "src/cwb/utils/cwb-huffcode.c" = c(
          list(
            ...
            insert_before = list("^(\\s*)for\\s*\\(i\\s=\\s0;\\si\\s<\\shc->length;\\si\\+\\+\\)\\s\\{\\s*$", "\\1int word, success;", 1L),
            replace = list(
        ...
        "src/cwb/cl/cl.h" = list(
          ...
          insert_before = list("#define\\sCL_STREAM_READ_BIN\\s+0", "#ifndef __MINGW__", 1L),
          insert_after = list("#define\\sCL_STREAM_READ_BIN\\s+0", "#endif", 1L),
