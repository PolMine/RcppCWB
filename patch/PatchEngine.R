      ...
      list(
        ...
        "src/cwb/utils/cwb-huffcode.c" = c(
          ...
          if (revision >= 1690) list(
            ...
            delete_line_before("^\\s*compute_code_lengths(Attribute\\s\\*attr,\\sHCD\\s\\*hc,\\schar\\s\\*fname\\)\\s*$", 1L, 1L),
            insert_before("^\\s*compute_code_lengths(Attribute\\s\\*attr,\\sHCD\\s\\*hc,\\schar\\s\\*fname\\)\\s*$", "int", 1L)
          )
        ),
