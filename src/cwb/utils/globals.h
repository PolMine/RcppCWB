...
/*    Rprintf("  -r <dir>  set registry directory (for -C flag)\n"); */
  ...
  Rprintf("  -L <att>  interpret blank lines as sentence boundaries stored in s-attribute <att>\n");
  Rprintf("  -n        alternative input format with all token lines numbered in first column\n");
  Rprintf("     * avoids ambiguities between token lines and XML tags\n");
  Rprintf("     * malformed tags and junk lines are fatal errors, undeclared tags are ignored\n");
  Rprintf("  -N <att>  numbered input format, but token numbers are encoded in p-attribute <att>\n");
  Rprintf("  -9        discard XML tags for all undeclared s-attributes (auto null-attributes)\n");
  ...
  Rprintf("     * valid charsets: ascii ; latin1 .. latin9 ; arabic, greek, hebrew, cyrillic ; utf8\n");
  Rprintf("     * iso-8859-1 .. iso-8859-15 are also accepted, but converted to canonical names above\n");
    ...
    if (end_pos >= encoder->start_pos) {
      ...
      if (encoder->store_values) {
        ...
        if (!(entry = cl_lexhash_find(encoder->lh, encoder->annot)))  {
          ...
          Rprintf(encoder->avs_fh, "%s%c", encoder->annot, '\0');
...
encode_generate_registry_file(char *registry_file)
{
  ...
  Rprintf(registry_fh, "# long descriptive name for the corpus\n"
                       "NAME \"\"\n"
                       "# corpus ID (must be lowercase in registry!)\n"
                       "ID   %s\n", registry_id);
  Rprintf(registry_fh, "# path to binary data files\n");
  path = cl_path_registry_quote(directory);
  Rprintf(registry_fh, "HOME %s\n", path);
  cl_free(path);
  Rprintf(registry_fh, "# optional info file (displayed by \"info;\" command in CQP)\n");
  path = cl_path_registry_quote(info_file);
  Rprintf(registry_fh, "INFO %s\n\n", path);
  cl_free(path);
  Rprintf(registry_fh, "# corpus properties provide additional information about the corpus:\n");
  /* lines marked with ##:: are NOT commented out, this is part of the normal registry format! */
  Rprintf(registry_fh, "##:: charset  = \"%s\" # character encoding of corpus data\n", encoding_charset_name);
  Rprintf(registry_fh, "##:: language = \"??\"     # insert ISO code for language (de, en, fr, ...)\n"
                       "\n\n");

  /* insert p-attributes into registry file */
  Rprintf(registry_fh, "##\n## p-attributes (token annotations)\n##\n\n");
  for (i = 0; i < p_encoder_ix; i++)
    Rprintf(registry_fh, "ATTRIBUTE %s\n", p_encoder[i].name);
