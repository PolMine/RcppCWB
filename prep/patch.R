...
replace <- list(
  ...
  # "src/cwb/cqp/groups.c" = list("^(\\s*)", "\\1*/", 1),
  # "src/cwb/cqp/groups.c" = list("^(\\s*)return ComputeGroupExternally(group); /* modifies Group object in place and returns pointer */", "\\1", 1),
  # "src/cwb/cqp/hash.c" = list("^(\\s*)-- a/src/cwb/cqp/hash.c", "\\1++ b/src/cwb/cqp/hash.c", 1),
  # "src/cwb/cqp/hash.c" = list("^(\\s*)", "\\1/*", 1),
  # "src/cwb/cqp/hash.c" = list("^(\\s*)for( ; n > 0 ; n++)		/* will exit on int overflow */", "\\1for( ; n > 0 ; n++)		", 1),
  # "src/cwb/cqp/hash.h" = list("^(\\s*)-- a/src/cwb/cqp/hash.h", "\\1++ b/src/cwb/cqp/hash.h", 1),
  # "src/cwb/cqp/hash.h" = list("^(\\s*)unsigned int hash_string(char *s);	/* get 32bit hash value for string */", "\\1/* #include "../cl/lexhash.h" */  /* added by AB */", 1),
  # "src/cwb/cqp/html-print.c" = list("^(\\s*)-- a/src/cwb/cqp/html-print.c", "\\1++ b/src/cwb/cqp/html-print.c", 1),
  # "src/cwb/cqp/html-print.c" = list("^(\\s*)char *s;", "\\1/* char *s; */", 1),
  # "src/cwb/cqp/html-print.c" = list("^(\\s*)s = "error";", "\\1/* s = "error"; */", 1),
  # "src/cwb/cqp/html-print.c" = list("^(\\s*)s = "error";", "\\1/* s = "error"; */", 1),
  # "src/cwb/cqp/html-print.c" = list("^(\\s*)AttributeList *strucs;", "\\1/* AttributeList *strucs; */", 1),
  # "src/cwb/cqp/html-print.c" = list("^(\\s*)strucs = cd->printStructureTags;", "\\1/* strucs = cd->printStructureTags; */", 1),
  # "src/cwb/cqp/latex-print.c" = list("^(\\s*)-- a/src/cwb/cqp/latex-print.c", "\\1++ b/src/cwb/cqp/latex-print.c", 1),
  # "src/cwb/cqp/latex-print.c" = list("^(\\s*)char *s;", "\\1/* char *s; */", 1),
  # "src/cwb/cqp/latex-print.c" = list("^(\\s*)s = "error";", "\\1/* s = "error"; */", 1),
  # "src/cwb/cqp/latex-print.c" = list("^(\\s*)s = "error";", "\\1/* s = "error"; */", 1),
  ...
  # "src/cwb/cqp/output.c" = list("^(\\s*)", "\\1*/", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)Rprintf(rd->stream, "%d matches.\n", cl->size);", "\\1Rprintf("%d matches.\n", cl->size);", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)vRprintf(format, ap);", "\\1Rprintf(format, ap);", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)Rprintf(rd->stream, "%d", cpos);", "\\1Rprintf("%d", cpos);", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)Rprintf(rd->stream, "%s", copy);", "\\1Rprintf("%s", copy);", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)Rprintf(rd->stream, "%s", string);", "\\1Rprintf("%s", string);", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)Rprintf(rd->stream, "-1");", "\\1Rprintf("-1");", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)Rprintf(rd->stream, " "); ", "\\1Rprintf(" "); ", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)Rprintf(rd->stream, "\t");", "\\1Rprintf("\t");", 1),
  # "src/cwb/cqp/output.c" = list("^(\\s*)Rprintf(rd->stream, "\n");", "\\1Rprintf("\n");", 1),
  # "src/cwb/cqp/output.h" = list("^(\\s*)-- a/src/cwb/cqp/output.h", "\\1++ b/src/cwb/cqp/output.h", 1),
  # "src/cwb/cqp/output.h" = list("^(\\s*)FILE *open_temporary_file(char *tmp_name_buffer);", "\\1/* FILE *open_temporary_file(char *tmp_name_buffer); */", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)-- a/src/cwb/cqp/parse_actions.c", "\\1++ b/src/cwb/cqp/parse_actions.c", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)Rprintf(rd->stream, "%s", s);", "\\1Rprintf("%s", s);", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)c->idlist.delete = left->pa_ref.delete;", "\\1c->idlist.del = left->pa_ref.del;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)left->pa_ref.delete = 0;", "\\1left->pa_ref.del = 0;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)node->idlist.delete = 0;", "\\1node->idlist.del = 0;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)res->idlist.delete = left->pa_ref.delete;", "\\1res->idlist.del = left->pa_ref.del;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)res->pa_ref.delete = auto_delete;", "\\1res->pa_ref.del = auto_delete;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)res->sa_ref.delete = auto_delete;", "\\1res->sa_ref.del = auto_delete;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)res->pa_ref.delete = 0;", "\\1res->pa_ref.del = 0;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)res->pa_ref.delete = auto_delete;", "\\1res->pa_ref.del = auto_delete;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)res->sa_ref.delete = 0;", "\\1res->sa_ref.del = 0;", 1),
  # "src/cwb/cqp/parse_actions.c" = list("^(\\s*)Rprintf(rd->stream, "%d\t%d\t%d\t%d\n", rg->start, rg->end, target, keyword);", "\\1Rprintf("%d\t%d\t%d\t%d\n", rg->start, rg->end, target, keyword);", 1),
  # "src/cwb/cqp/parser.y" = list("^(\\s*)-- a/src/cwb/cqp/parser.y", "\\1++ b/src/cwb/cqp/parser.y", 1),
  # "src/cwb/cqp/parser.y" = list("^(\\s*)int ok;", "\\1int ok __attribute__((unused));", 1),
  # "src/cwb/cqp/parser.y" = list("^(\\s*)int ok;", "\\1int ok __attribute__((unused));", 1),
  # "src/cwb/cqp/parser.y" = list("^(\\s*)ok = SortSubcorpus($2, $3, ($4 >= 1) ? $4 : 1, &($5));", "\\1SortSubcorpus($2, $3, ($4 >= 1) ? $4 : 1, &($5));", 1),
  # "src/cwb/cqp/ranges.c" = list("^(\\s*)-- a/src/cwb/cqp/ranges.c", "\\1++ b/src/cwb/cqp/ranges.c", 1),
