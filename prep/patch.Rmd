...
```{r}
cwb_copy_report <- cwb_fresh_copy(cwb_dir = cwb_dir_svn, repodir = repodir, source_branch = "dev")
cwb_copy_report
cwb_revision_branch <- cwb_copy_report[["bracnh"]]
```
...
for (i in 1L:length(delete_line_before)){
  ...
  message(
     sprintf(
       "file: %s | regex: %s | match: %d | lines: %d ... ",
       names(delete_line_before)[[i]],
       delete_line_before[[i]][[1]],
       which_position,
       times
      ),
     appendLF = FALSE
  ...
  if (!file.exists(fname)){
    message("skipping operation - file ", fname, " does not exist")
    next
  }
...
insert_before <- list(
  ...
  "src/cwb/config/platform/linux" = list("##\\sCPU\\sarchitecture", c("## require position-independent code", "CFLAGS += -fPIC", ""), 1L)
)
...
for (i in 1L:length(insert_before)){
  ...
  message(
    sprintf(
      "file: %s | regex: %s | match: %d | insertion: %s ... ",
      fname,
      insert_before[[i]][[1]],
      which_position,
      paste(insert_before[[i]][[2]], collapse = "///")
    ),
    appendLF = FALSE
  )
  
  if (!file.exists(fname)){
    message("FAIL - file does not exist")
    next
  }
...
for (i in 1L:length(insert_after)){
  ...
  message(
    sprintf(
      ...
      insert_after[[i]][[1]],
      which_position,
      paste(insert_after[[i]][[2]], collapse = "///")
    ),
    appendLF = FALSE
  )
...
replace <- list(
  ...
  "src/cwb/config/platform/darwin" = list("^(CFLAGS\\s*=.*?)\\s+-march=native\\s+(.*?)$", "\\1 \\2", 1L),
  "src/cwb/config/platform/darwin-port-core2" = list("^(CFLAGS\\s*=.*?)\\s+-march=core2\\s+(.*?)$", "\\1 \\2", 1L),
  "src/cwb/config/platform/darwin-universal" = list("^(CFLAGS\\s*=.*?)\\s+-march=core2\\s+(.*?)$", "\\1 \\2", 1L),
  "src/cwb/config/platform/darwin-brew" = list("^(CFLAGS\\s*=.*?)\\s+-march=native\\s+(.*?)$", "\\1 \\2", 1L),
  "src/cwb/config/platform/linux-opteron" = list("^(CFLAGS\\s*=.*?)\\s+-march=opteron\\s+(.*?)$", "\\1 \\2", 1L),
  "src/cwb/config/platform/linux" = list("^(CFLAGS\\s*=.*?)\\s+-march=native\\s+(.*?)$", "\\1 \\2", 1L),
  "src/cwb/config/platform/darwin" = list("^READLINE_DEFINES\\s:=.*?$", "READLINE_DEFINES =", 1L),
  "src/cwb/config/platform/darwin" = list("^READLINE_LIBS\\s:=.*?$", "READLINE_LIBS =", 1L),
  "src/cwb/config/platform/unix" = list("^AR\\s+=\\s+ar\\scq\\s*$", "AR = ar", 1L),
  "src/cwb/definitions.mk" = list("(\\$\\(error\\sConfiguration\\svariable\\sRANLIB)", "# \\1", 1L)
)
...
for (i in 1L:length(replace)){
  ...
  message(
    sprintf(
      "file: %s | regex: %s | match: %d | replacement: %s ... ",
      fname,
      replace[[i]][[1]],
      replace[[i]][[3]],
      paste(replace[[i]][[2]], collapse = "///")
    ),
    appendLF = FALSE
  )
...
lapply(
  ...
  function(f){
    ...
    if (!file.exists(fname)){
      message(sprintf("FAIL - file %s does not exist", f))
      next
    }
