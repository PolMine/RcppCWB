...
git_cmd <- sprintf(
  ...
  "':!*endian.h' ':!*Makefile' ':!*endian2.h' ':!*lex.creg.c' ':!*registry.tab.c' ':!*registry.tab.h'" 
)
...
git_cmd <- sprintf(
  "git diff %s..%s . %s", to_compare[["sha"]], last_devbranch_commit[["sha"]],
  "':!*_options.h' ':!*parser.tab.c' ':!*parser.tab.h' ':!*lex.yy.c'"
)
result <- system(git_cmd, intern = TRUE)
output <- paste(result, collapse = "\n")
cat(output, file = path(repodir, "prep", "gitdiff_cqp.txt"))
cat(output)
```
...
git_cmd <- sprintf(
  "git diff %s..%s . %s", to_compare[["sha"]], last_devbranch_commit[["sha"]],
  "':!*_cwb_encode.c' ':!*_cwb_makeall.c' ':!*_cwb_huffcode.c' ':!*_cwb_compress_rdx.c'"
)
result <- system(git_cmd, intern = TRUE)
cat(paste(result, collapse = "\n"))
```
