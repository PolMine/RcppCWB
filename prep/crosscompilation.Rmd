...
systems. Indeed, portability is the major reason for the effort
that went into the package.

The essential idea of the package is that pre-compiled binaries 
of the corpus library and further libraries are available for Windows
via a  GitHub repository (github.com/PolMine/libcl). During the installation,
the binaries are downloaded, and the Rcpp code is compiled against
these binaries.

For this, a pre-compiled version of the CWB is necessary. The annex explains
the procedure that has been used.

On Linux and macOS, offical binaries of the CWB (that include libcl.a)
are downloaded, if the CWB is not yet installed. To identify of the libraries
and header files are already present, the tool cwb-config is used.
